...
func initLoIdFlags(cmd *cobra.Command) {
...
func ListObjId(cmd *cobra.Command, args []string) {
	...
	if len(prefix) > 0 {
		...
		ListObjIds(srcBucket,marker,prefix,maxKey,maxPage,maxLoop)
	}
}

func ListObjIds(bucket string, marker string,prefix string,maxKey int64,maxPage int,maxLoop int) {
	...
	if srcS3:= mosesbc.CreateS3Session("check","source"); srcS3 != nil {
		...
		mosesbc.ListObjIds(request, maxLoop, maxPage)
	} else {
